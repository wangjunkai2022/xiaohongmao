...
public class MaterialToolbar extends Toolbar {
    ...
    private void layoutTitleCenteredHorizontally(View view, Pair<Integer, Integer> pair) {
        ...
        int max = Math.max(Math.max(((Integer) pair.first).intValue() - i4, 0), Math.max(i10 - ((Integer) pair.second).intValue(), 0));
        if (max > 0) {
            i4 += max;
            i10 -= max;
            view.measure(View.MeasureSpec.makeMeasureSpec(i10 - i4, 1073741824), view.getMeasuredHeightAndState());
        }
        view.layout(i4, view.getTop(), i10, view.getBottom());
    }
