...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    private static void updateTopBottomPadding(@NonNull View view, int i4, int i10) {
        if (ViewCompat.isPaddingRelative(view)) {
            ViewCompat.setPaddingRelative(view, ViewCompat.getPaddingStart(view), i4, ViewCompat.getPaddingEnd(view), i10);
        } else {
            view.setPadding(view.getPaddingLeft(), i4, view.getPaddingRight(), i10);
        }
    }

    private boolean updateViewsWithinLayout(int i4, int i10, int i11) {
        boolean z9;
    ...
    public void animateContentIn(int i4, int i10) {
        this.messageView.setAlpha(0.0f);
        long j4 = i10;
        long j10 = i4;
        this.messageView.animate().alpha(1.0f).setDuration(j4).setStartDelay(j10).start();
        if (this.actionView.getVisibility() == 0) {
            this.actionView.setAlpha(0.0f);
            this.actionView.animate().alpha(1.0f).setDuration(j4).setStartDelay(j10).start();
        }
    }

    @Override // com.google.android.material.snackbar.ContentViewCallback
    public void animateContentOut(int i4, int i10) {
        this.messageView.setAlpha(1.0f);
        long j4 = i10;
        long j10 = i4;
        this.messageView.animate().alpha(0.0f).setDuration(j4).setStartDelay(j10).start();
        if (this.actionView.getVisibility() == 0) {
            this.actionView.setAlpha(1.0f);
            this.actionView.animate().alpha(0.0f).setDuration(j4).setStartDelay(j10).start();
        }
    }
