...
public class MemberComparator implements Comparator<DeclarationDescriptor> {
    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        ...
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int i4 = 0; i4 < Math.min(valueParameters.size(), valueParameters2.size()); i4++) {
                DescriptorRenderer descriptorRenderer3 = RENDERER;
                int compareTo3 = descriptorRenderer3.renderType(valueParameters.get(i4).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(i4).getType()));
                if (compareTo3 != 0) {
                    return compareTo3;
                }
            }
            ...
            for (int i10 = 0; i10 < Math.min(typeParameters.size(), typeParameters2.size()); i10++) {
                List<KotlinType> upperBounds = typeParameters.get(i10).getUpperBounds();
                List<KotlinType> upperBounds2 = typeParameters2.get(i10).getUpperBounds();
                ...
                for (int i11 = 0; i11 < upperBounds.size(); i11++) {
                    ...
                    int compareTo4 = descriptorRenderer4.renderType(upperBounds.get(i11)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(i11)));
                    if (compareTo4 != 0) {
                        return compareTo4;
                    }
                }
