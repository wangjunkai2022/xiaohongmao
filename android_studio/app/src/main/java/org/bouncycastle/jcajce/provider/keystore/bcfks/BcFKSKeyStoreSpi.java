...
public class BcFKSKeyStoreSpi extends KeyStoreSpi {
    ...
    private byte[] generateKey(KeyDerivationFunc keyDerivationFunc, String str, char[] cArr, int i4) throws IOException {
        ...
        if (MiscObjectIdentifiers.id_scrypt.equals((ASN1Primitive) keyDerivationFunc.getAlgorithm())) {
            ...
            if (scryptParams.getKeyLength() != null) {
                i4 = scryptParams.getKeyLength().intValue();
            } else if (i4 == -1) {
                throw new IOException("no keyLength found in ScryptParams");
            }
            return SCrypt.generate(Arrays.concatenate(PKCS12PasswordToBytes, PKCS12PasswordToBytes2), scryptParams.getSalt(), scryptParams.getCostParameter().intValue(), scryptParams.getBlockSize().intValue(), scryptParams.getBlockSize().intValue(), i4);
            ...
            if (pBKDF2Params.getKeyLength() != null) {
                i4 = pBKDF2Params.getKeyLength().intValue();
            } else if (i4 == -1) {
            ...
            if (pBKDF2Params.getPrf().getAlgorithm().equals((ASN1Primitive) PKCSObjectIdentifiers.id_hmacWithSHA512)) {
                ...
                return ((KeyParameter) pKCS5S2ParametersGenerator.generateDerivedParameters(i4 * 8)).getKey();
                ...
                return ((KeyParameter) pKCS5S2ParametersGenerator2.generateDerivedParameters(i4 * 8)).getKey();
            } else {
    ...
    private KeyDerivationFunc generatePkbdAlgorithmIdentifier(ASN1ObjectIdentifier aSN1ObjectIdentifier, int i4) {
        ...
        if (aSN1ObjectIdentifier2.equals((ASN1Primitive) aSN1ObjectIdentifier)) {
            return new KeyDerivationFunc(aSN1ObjectIdentifier2, new PBKDF2Params(bArr, a.f93880a, i4, new AlgorithmIdentifier(PKCSObjectIdentifiers.id_hmacWithSHA512, DERNull.INSTANCE)));
        }
        throw new IllegalStateException("unknown derivation algorithm: " + aSN1ObjectIdentifier);
    }

    private KeyDerivationFunc generatePkbdAlgorithmIdentifier(KeyDerivationFunc keyDerivationFunc, int i4) {
        ...
        if (equals) {
            ...
            getDefaultSecureRandom().nextBytes(bArr);
            return new KeyDerivationFunc(aSN1ObjectIdentifier, new ScryptParams(bArr, scryptParams.getCostParameter(), scryptParams.getBlockSize(), scryptParams.getParallelizationParameter(), BigInteger.valueOf(i4)));
        }
        PBKDF2Params pBKDF2Params = PBKDF2Params.getInstance(parameters);
        byte[] bArr2 = new byte[pBKDF2Params.getSalt().length];
        getDefaultSecureRandom().nextBytes(bArr2);
        return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, new PBKDF2Params(bArr2, pBKDF2Params.getIterationCount().intValue(), i4, pBKDF2Params.getPrf()));
    }

    private KeyDerivationFunc generatePkbdAlgorithmIdentifier(PBKDFConfig pBKDFConfig, int i4) {
        ASN1ObjectIdentifier aSN1ObjectIdentifier = MiscObjectIdentifiers.id_scrypt;
        if (aSN1ObjectIdentifier.equals((ASN1Primitive) pBKDFConfig.getAlgorithm())) {
            ...
            getDefaultSecureRandom().nextBytes(bArr);
            return new KeyDerivationFunc(aSN1ObjectIdentifier, new ScryptParams(bArr, scryptConfig.getCostParameter(), scryptConfig.getBlockSize(), scryptConfig.getParallelizationParameter(), i4));
        }
        PBKDF2Config pBKDF2Config = (PBKDF2Config) pBKDFConfig;
        byte[] bArr2 = new byte[pBKDF2Config.getSaltLength()];
        getDefaultSecureRandom().nextBytes(bArr2);
        return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, new PBKDF2Params(bArr2, pBKDF2Config.getIterationCount(), i4, pBKDF2Config.getPRF()));
    }
        ...
        if (objectData != null) {
            if (objectData.getType().equals(PRIVATE_KEY) || objectData.getType().equals(PROTECTED_PRIVATE_KEY)) {
                ...
                for (int i4 = 0; i4 != length; i4++) {
                    x509CertificateArr[i4] = decodeCertificate(certificateChain[i4]);
                }
            ...
            if (integrityCheck.getType() == 0) {
                ...
                try {
                    ...
                    if (this.validator == null) {
                        ...
                        X509Certificate[] x509CertificateArr = new X509Certificate[length];
                        for (int i4 = 0; i4 != length; i4++) {
                            x509CertificateArr[i4] = (X509Certificate) createCertificateFactory.generateCertificate(new ByteArrayInputStream(certificates[i4].getEncoded()));
                        }
        ...
        if (loadStoreParameter instanceof BCFKSStoreParameter) {
            ...
            try {
                ...
                if (storeCertificates != null) {
                    ...
                    for (int i4 = 0; i4 != length; i4++) {
                        certificateArr[i4] = org.bouncycastle.asn1.x509.Certificate.getInstance(storeCertificates[i4].getEncoded());
                    }
