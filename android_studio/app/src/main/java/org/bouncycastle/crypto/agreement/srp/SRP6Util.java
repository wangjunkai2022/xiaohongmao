...
public class SRP6Util {
    ...
    private static byte[] getPadded(BigInteger bigInteger, int i4) {
        byte[] asUnsignedByteArray = BigIntegers.asUnsignedByteArray(bigInteger);
        if (asUnsignedByteArray.length < i4) {
            byte[] bArr = new byte[i4];
            System.arraycopy(asUnsignedByteArray, 0, bArr, i4 - asUnsignedByteArray.length, asUnsignedByteArray.length);
            return bArr;
        }
