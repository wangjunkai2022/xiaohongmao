...
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    public HMacSP800DRBG(Mac mac, int i4, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        if (i4 > Utils.getMaxSecurityStrength(mac)) {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z9) {
        ...
        if (length <= 262144) {
            ...
            for (int i4 = 0; i4 < length3; i4++) {
                ...
                this._hMac.doFinal(this._V, 0);
                byte[] bArr5 = this._V;
                System.arraycopy(bArr5, 0, bArr3, bArr5.length * i4, bArr5.length);
            }
            byte[] bArr6 = this._V;
