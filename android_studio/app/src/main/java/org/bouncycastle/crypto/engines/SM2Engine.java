...
public class SM2Engine {
    ...
    private byte[] decrypt(byte[] bArr, int i4, int i10) throws InvalidCipherTextException {
        int i11;
        ...
        if (mode == mode2) {
            System.arraycopy(bArr, i4 + i12 + digestSize, bArr3, 0, i13);
        } else {
            System.arraycopy(bArr, i4 + i12, bArr3, 0, i13);
        }
        ...
        if (this.mode == mode2) {
            ...
            for (int i14 = 0; i14 != digestSize2; i14++) {
                i11 |= bArr4[i14] ^ bArr[(i4 + i12) + i14];
            }
        } else {
            i11 = 0;
            for (int i15 = 0; i15 != digestSize2; i15++) {
                i11 |= bArr4[i15] ^ bArr[((i4 + i12) + i13) + i15];
            }
        }
    ...
    private byte[] encrypt(byte[] bArr, int i4, int i10) throws InvalidCipherTextException {
        byte[] encoded;
        ECPoint normalize;
        ...
        } while (notEncrypted(bArr2, bArr, i4));
    ...
    private boolean notEncrypted(byte[] bArr, byte[] bArr2, int i4) {
        for (int i10 = 0; i10 != bArr.length; i10++) {
            if (bArr[i10] != bArr2[i4 + i10]) {
                return false;
            }
        }
        return true;
    ...
    private void xor(byte[] bArr, byte[] bArr2, int i4, int i10) {
    ...
    public int getOutputSize(int i4) {
        return (this.curveLength * 2) + 1 + i4 + this.digest.getDigestSize();
    }
    ...
    public byte[] processBlock(byte[] bArr, int i4, int i10) throws InvalidCipherTextException {
        return this.forEncryption ? encrypt(bArr, i4, i10) : decrypt(bArr, i4, i10);
    }
}
