...
public class DSTU4145Signer implements DSAExt {
    ...
    private static BigInteger truncate(BigInteger bigInteger, int i4) {
        return bigInteger.bitLength() > i4 ? bigInteger.mod(ONE.shiftLeft(i4)) : bigInteger;
    }
    ...
    public BigInteger[] generateSignature(byte[] bArr) {
        ...
        while (true) {
            BigInteger generateRandomInteger = generateRandomInteger(n9, this.random);
            ...
            if (!affineXCoord.isZero()) {
                BigInteger fieldElement2Integer = fieldElement2Integer(n9, hash2FieldElement.multiply(affineXCoord));
                if (fieldElement2Integer.signum() != 0) {
                    BigInteger mod = fieldElement2Integer.multiply(d4).add(generateRandomInteger).mod(n9);
                    if (mod.signum() != 0) {
    ...
    public boolean verifySignature(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        ECDomainParameters parameters = this.key.getParameters();
        BigInteger n9 = parameters.getN();
        if (bigInteger.compareTo(n9) >= 0 || bigInteger2.compareTo(n9) >= 0) {
            return false;
        }
        ECCurve curve = parameters.getCurve();
        ECFieldElement hash2FieldElement = hash2FieldElement(curve, bArr);
        ...
        return !normalize.isInfinity() && fieldElement2Integer(n9, hash2FieldElement.multiply(normalize.getAffineXCoord())).compareTo(bigInteger) == 0;
    }
}
