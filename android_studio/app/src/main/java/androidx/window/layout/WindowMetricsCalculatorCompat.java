...
public final class WindowMetricsCalculatorCompat implements WindowMetricsCalculator {
    ...
    @g
    public WindowMetrics computeCurrentWindowMetrics(@g Activity activity) {
    ...
    @Override // androidx.window.layout.WindowMetricsCalculator
    @g
    public WindowMetrics computeMaximumWindowMetrics(@g Activity activity) {
        Rect rect;
        Intrinsics.checkNotNullParameter(activity, "activity");
    ...
    @RequiresApi(14)
    @g
    public final Rect computeWindowBoundsIceCreamSandwich$window_release(@g Activity activity) {
        int i4;
        Intrinsics.checkNotNullParameter(activity, "activity");
        ...
        Rect rect = new Rect();
        int i10 = realSizeForDisplay$window_release.x;
        if (i10 != 0 && (i4 = realSizeForDisplay$window_release.y) != 0) {
            rect.right = i10;
            rect.bottom = i4;
        } else {
    ...
    @RequiresApi(24)
    @g
    public final Rect computeWindowBoundsN$window_release(@g Activity activity) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Rect rect = new Rect();
        Display defaultDisplay = activity.getWindowManager().getDefaultDisplay();
        defaultDisplay.getRectSize(rect);
        if (!ActivityCompatHelperApi24.INSTANCE.isInMultiWindowMode(activity)) {
            ...
            if (i4 + navigationBarHeight == realSizeForDisplay$window_release.y) {
                ...
                int i10 = rect.right;
                if (i10 + navigationBarHeight == realSizeForDisplay$window_release.x) {
                    rect.right = i10 + navigationBarHeight;
                }
            }
        }
        return rect;
    ...
    public final Rect computeWindowBoundsP$window_release(@g Activity activity) {
        ...
        if (!activityCompatHelperApi24.isInMultiWindowMode(activity)) {
            ...
            if (i4 + navigationBarHeight == point.y) {
                ...
                int i10 = rect.right;
                if (i10 + navigationBarHeight == point.x) {
                    rect.right = i10 + navigationBarHeight;
        ...
        if ((rect.width() < point.x || rect.height() < point.y) && !activityCompatHelperApi24.isInMultiWindowMode(activity) && (cutoutForDisplay = getCutoutForDisplay(currentDisplay)) != null) {
            ...
            if (i11 == displayCompatHelperApi28.safeInsetLeft(cutoutForDisplay)) {
                rect.left = 0;
            }
    ...
    @g
    @SuppressLint({"BanUncheckedReflection", "BlockedPrivateApi"})
    public final Rect computeWindowBoundsQ$window_release(@g Activity activity) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Configuration configuration = activity.getResources().getConfiguration();
        try {
            Field declaredField = Configuration.class.getDeclaredField("windowConfiguration");
    ...
    @g
    @VisibleForTesting
    public final Point getRealSizeForDisplay$window_release(@g Display display) {
