...
public final class DispatchQueue {
    ...
    public final void dispatchAndEnqueue(@m8.g CoroutineContext context, @m8.g final Runnable runnable) {
        Intrinsics.checkNotNullParameter(context, "context");
        Intrinsics.checkNotNullParameter(runnable, "runnable");
        u2 p02 = i1.e().p0();
        if (!p02.isDispatchNeeded(context) && !canRun()) {
            enqueue(runnable);
        } else {
            p02.dispatch(context, new Runnable() { // from class: androidx.lifecycle.c
                @Override // java.lang.Runnable
                public final void run() {
                    DispatchQueue.m23dispatchAndEnqueue$lambda2$lambda1(DispatchQueue.this, runnable);
                }
            });
        }
    }
