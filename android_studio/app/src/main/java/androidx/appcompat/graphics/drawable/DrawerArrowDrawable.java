...
public class DrawerArrowDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        float lerp4 = lerp(z9 ? 0.0f : -180.0f, z9 ? 180.0f : 0.0f, this.mProgress);
        double d4 = lerp;
        double d10 = lerp3;
        boolean z10 = z9;
        float round2 = (float) Math.round(Math.cos(d10) * d4);
        float round3 = (float) Math.round(d4 * Math.sin(d10));
        this.mPath.rewind();
        ...
        if (this.mSpin) {
            canvas.rotate(lerp4 * (this.mVerticalMirror ^ z10 ? -1 : 1));
        } else if (z10) {
