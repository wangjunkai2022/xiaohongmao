...
public class InvalidationTracker {
    ...
    private static final String[] TRIGGERS = {"UPDATE", AsyncHttpDelete.f44553o, "INSERT"};
            ...
            synchronized (this) {
                if (this.mNeedsSync) {
                    ...
                    for (int i4 = 0; i4 < length; i4++) {
                        int i10 = 1;
                        boolean z9 = this.mTableObservers[i4] > 0;
    ...
    public static class ObserverWrapper {
        ...
        void notifyByTableInvalidStatus(Set<Integer> set) {
            ...
            for (int i4 = 0; i4 < length; i4++) {
                if (set.contains(Integer.valueOf(this.mTableIds[i4]))) {
                    if (length == 1) {
        ...
        void notifyByTableNames(String[] strArr) {
            ...
            if (this.mTableNames.length == 1) {
                ...
                while (true) {
                    if (i4 >= length) {
                        break;
                    } else if (strArr[i4].equalsIgnoreCase(this.mTableNames[0])) {
    ...
    private void startTrackingTable(SupportSQLiteDatabase supportSQLiteDatabase, int i4) {
        String[] strArr;
    ...
    private void stopTrackingTable(SupportSQLiteDatabase supportSQLiteDatabase, int i4) {
        String[] strArr;
        String str = this.mTableNames[i4];
        StringBuilder sb = new StringBuilder();
        for (String str2 : TRIGGERS) {
        ...
        try {
            ...
            try {
                synchronized (this.mSyncTriggersLock) {
                    ...
                    for (int i4 = 0; i4 < length; i4++) {
                        int i10 = tablesToSync[i4];
                        if (i10 == 1) {
                            startTrackingTable(supportSQLiteDatabase, i4);
                        } else if (i10 == 2) {
                            stopTrackingTable(supportSQLiteDatabase, i4);
                        }
                    }
    ...
    public <T> LiveData<T> createLiveData(String[] strArr, boolean z9, Callable<T> callable) {
        return this.mInvalidationLiveDataContainer.create(validateAndResolveTableNames(strArr), z9, callable);
    }
