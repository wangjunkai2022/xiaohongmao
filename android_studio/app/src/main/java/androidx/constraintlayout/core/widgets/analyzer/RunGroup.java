...
public class RunGroup {
        ...
        for (int i4 = 0; i4 < size; i4++) {
            ...
            if (dependency instanceof DependencyNode) {
                ...
                if (dependencyNode2.run != widgetRun) {
                    j10 = Math.min(j10, traverseEnd(dependencyNode2, dependencyNode2.margin + j4));
                }
            }
        }
        if (dependencyNode == widgetRun.end) {
            long wrapDimension = j4 - widgetRun.getWrapDimension();
            return Math.min(Math.min(j10, traverseEnd(widgetRun.start, wrapDimension)), wrapDimension - widgetRun.start.margin);
        }
        return j10;
    }
    ...
    private long traverseStart(DependencyNode dependencyNode, long j4) {
        ...
        for (int i4 = 0; i4 < size; i4++) {
            ...
            if (dependency instanceof DependencyNode) {
                DependencyNode dependencyNode2 = (DependencyNode) dependency;
                if (dependencyNode2.run != widgetRun) {
                    j10 = Math.max(j10, traverseStart(dependencyNode2, dependencyNode2.margin + j4));
                }
            }
        }
        if (dependencyNode == widgetRun.start) {
            long wrapDimension = j4 + widgetRun.getWrapDimension();
            return Math.max(Math.max(j10, traverseStart(widgetRun.end, wrapDimension)), wrapDimension - widgetRun.end.margin);
        }
        return j10;
    }
    ...
    public long computeWrapSize(ConstraintWidgetContainer constraintWidgetContainer, int i4) {
        ...
        if (contains && contains2) {
            ...
            long j10 = ((-traverseEnd) - wrapDimension2) - i12;
            if (j10 >= i12) {
                j10 -= i12;
            }
            float biasPercent = widgetRun2.widget.getBiasPercent(i4);
            float f10 = (float) (biasPercent > 0.0f ? (((float) j10) / biasPercent) + (((float) j4) / (1.0f - biasPercent)) : 0L);
            long j11 = (f10 * biasPercent) + 0.5f + wrapDimension2 + (f10 * (1.0f - biasPercent)) + 0.5f;
            WidgetRun widgetRun3 = this.firstRun;
            wrapDimension = widgetRun3.start.margin + j11;
