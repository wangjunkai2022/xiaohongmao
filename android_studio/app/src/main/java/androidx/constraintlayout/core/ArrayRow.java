...
public class ArrayRow implements LinearSystem.Row {
    ...
    public interface ArrayRowVariables {
        void add(SolverVariable solverVariable, float f10, boolean z9);
    ...
    private SolverVariable pickPivotInVariables(boolean[] zArr, SolverVariable solverVariable) {
        ...
        for (int i4 = 0; i4 < currentSize; i4++) {
            ...
            if (variableValue < 0.0f) {
                ...
                if ((zArr == null || !zArr[variable.id]) && variable != solverVariable && (((type = variable.mType) == SolverVariable.Type.SLACK || type == SolverVariable.Type.ERROR) && variableValue < f10)) {
                    f10 = variableValue;
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public ArrayRow createRowCentering(SolverVariable solverVariable, SolverVariable solverVariable2, int i4, float f10, SolverVariable solverVariable3, SolverVariable solverVariable4, int i10) {
        ...
        if (f10 == 0.5f) {
            ...
            if (i4 > 0 || i10 > 0) {
                this.constantValue = ((-i4) * f11) + (i10 * f10);
            }
        }
        return this;
    }
    ...
    public ArrayRow createRowEqualDimension(float f10, float f11, float f12, SolverVariable solverVariable, int i4, SolverVariable solverVariable2, int i10, SolverVariable solverVariable3, int i11, SolverVariable solverVariable4, int i12) {
        if (f11 != 0.0f && f10 != f12) {
            float f13 = (f10 / f11) / (f12 / f11);
            this.constantValue = ((-i4) - i10) + (i11 * f13) + (i12 * f13);
            this.variables.put(solverVariable, 1.0f);
            this.variables.put(solverVariable2, -1.0f);
            this.variables.put(solverVariable4, f13);
            this.variables.put(solverVariable3, -f13);
        } else {
            this.constantValue = ((-i4) - i10) + i11 + i12;
            this.variables.put(solverVariable, 1.0f);
            this.variables.put(solverVariable2, -1.0f);
            this.variables.put(solverVariable4, 1.0f);
            this.variables.put(solverVariable3, -1.0f);
    ...
    */
    ...
    public void updateFromRow(LinearSystem linearSystem, ArrayRow arrayRow, boolean z9) {
        this.constantValue += arrayRow.constantValue * this.variables.use(arrayRow, z9);
        if (z9) {
    ...
    public void updateFromSynonymVariable(LinearSystem linearSystem, SolverVariable solverVariable, boolean z9) {
        ...
        this.variables.add(linearSystem.mCache.mIndexedVariables[solverVariable.synonym], f10, z9);
        if (LinearSystem.SIMPLIFY_SYNONYMS && this.variables.getCurrentSize() == 0) {
            this.isSimpleDefinition = true;
            linearSystem.hasSimpleDefinition = true;
        }
